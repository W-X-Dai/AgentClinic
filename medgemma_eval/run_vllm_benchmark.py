"""Run AgentClinic-NEJM evaluation through a running vLLM server."""
...
DEFAULT_RESULTS = Path(__file__).resolve().parent / "results" / "vllm-cases.jsonl"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--modes", default="context_only")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if set(modes) & IMAGE_MODES and not args.vision_capable:
        raise SystemExit("image modes require --vision-capable")
    if args.retries < 1:
        raise SystemExit("--retries must be at least 1")

    api_key = os.environ.get("VLLM_API_KEY", "").strip()
    client = VLLMClient(args.base_url, args.timeout, api_key)
    served_models = client.models()
    if not served_models:
        raise SystemExit("vLLM reports no served models")
    model = args.model or served_models[0]
    if model not in served_models:
        raise SystemExit(f"requested model {model!r} not in {served_models}")
    vllm_version = client.version()
    ...
    print(
        ...
        f"model={model}, vLLM={vllm_version or 'unknown'}, modes={','.join(modes)}"
    )
                            ...
                            record.update(
                                {
                                    ...
                                    "candidates_token_count": response.completion_tokens,
                                    "total_token_count": response.total_tokens,
                                    "response_model_version": response.response_model,
